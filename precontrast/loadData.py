...
# -------------------------------------------------------------------------------------------
        ...
        # datasets = scipy.io.loadmat('data/precontrast_datasets.mat')

        # Modify: use to visualize example testsets
        datasets = scipy.io.loadmat('data/precontrast_testsets.mat')
        ...
        if self.split == 'train':
            ...
            self.inputs = np.stack(((self.TI - self.TI_mean)/self.TI_std,(self.signal - self.signal_mean)/self.signal_std),axis=-1)
    ...
    def __getitem__(self,index):
        if self.split == 'train':
            return self.inputs[index],self.T1[index],self.signal[index],self.TI[index]
        if self.split == 'validation':
            return self.inputs[index],self.T1[index],self.signal[index],self.TI[index],self.myoMask[index]
        if self.split == 'test':
            return self.inputs[index],self.T1[index],self.signal[index],self.TI[index],self.myoMask[index]
